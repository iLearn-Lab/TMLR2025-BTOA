...
class VAE(nn.Module):
    def __init__(self, z_dim, args=None, use_cuda=False, prior_dist=dist.Normal(), q_dist=dist.Normal(),
        ...
        if self.args.save in ['ETTm2','ETTh2']:
            self.encoder = ConvEncoder_60(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_60(z_dim)
        elif self.args.save in ['Illness']:
            self.encoder = ConvEncoder_336(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_336(z_dim)
        elif self.args.save in ['ETTh1','ETTm1']:
            self.encoder = ConvEncoder_512(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_512(z_dim)
        elif self.args.save == 'ECL':
            self.encoder = ConvEncoder_ECL(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_ECL(z_dim)
        elif self.args.save == 'WTH' :
            self.encoder = ConvEncoder_WTH(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_WTH(z_dim)  
        elif self.args.save == 'Weather' :
            
            self.encoder = ConvEncoder_Weather(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_Weather(z_dim)      
        elif self.args.save == 'traffic' :
            self.encoder = ConvEncoder_traffic(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_traffic(z_dim) 
        elif self.args.save == 'Exchange' :
            self.encoder = ConvEncoder_Exchange(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_Exchange(z_dim)              
        elif self.args.saveset == 'ieee_small' or self.args.saveset == 'ieee_big' or self.args.saveset == 'dalia':
            self.encoder = ConvEncoder_ieeesmall(z_dim * self.q_dist.nparams)
            self.decoder = ConvDecoder_ieeesmall(z_dim)        
        elif self.args.saveset == 'ecg':
...
def train_VAE(train_loader, args, DEVICE):
    ...
    # setup the optimizer
    optimizer = optim.Adam(vae.parameters(), lr=args.VAE_learning_rate)
